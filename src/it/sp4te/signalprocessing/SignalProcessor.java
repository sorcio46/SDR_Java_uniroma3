...
public class SignalProcessor {
	...
	public static Signal bandPassFilter(double bandh, double bandl) {
		int numCampioni=5;
		Complex[] values = new Complex[numCampioni];
		int simmetria = (numCampioni) / 2;
		
		//Un filtro passa banda con lower band Bl e upper band Bh
		//non è altro che la differenza di due filtr sinc
		for(int n = - simmetria; n <= simmetria; n++){
			double realval = 2* bandh * sinc(n, 2 * bandh) -2* bandl * sinc(n, 2 * bandl);
